function varargout = plotWell2(G, W, varargin)
%An expanded version of MRST's plotWell function. Allows for plotting on
%existing axes.
%
% SYNOPSIS:
...
%
%  'Parent' - Axes used for plotting. Defaults to current axes.
%
% RETURNS:
...
%{
Modified by Christoph Scherounigg
...
for w = 1 : nW
   ...
   htop(2*(w-1) + 1) = patch(prm.Parent, ...
                             'Faces',  (1 : numel(nodes)/3), ...
                             'Vertices',  nodes,             ...
                             'EdgeColor', 'k',               ...
                             'FaceColor', color,         ...
   ...
   hline(w) = plot3(prm.Parent, c(:,1), c(:,2), c(:,3), ...
